...
const HexaCalculator = () => {
  ...
  const [nitrates, setNitrates] = useState(null);
  const [arsenic, setArsenic] = useState(null);
  const [calculationResult, setCalculationResult] = useState(null);
  const [errorMessage, setErrorMessage] = useState("");
  const [errorMessage1, setErrorMessage1] = useState("");
  const [ironError, setIronError] = useState("");
  const [manganeseError, setManganeseError] = useState("");
  const [nitratesError, setNitratesError] = useState("");
  const [arsenicError, setArsenicError] = useState("");
  ...
  const handleHardnessChange = (text) => {
    ...
    if ((!isNaN(value) && value < 0) || value > 800) {
      setErrorMessage("Please Enter hardness less then 800 ppm");
    } else {
      setErrorMessage("");
    }
    setHardness(text);
  ...
  const handleIron = (text) => {
    const value = parseFloat(text);
    if ((!isNaN(value) && value < 0) || value > 5) {
      setIronError("Iron should be less than 5 ppm");
    } else {
      setIronError("");
    }
    setIron(text);
  ...
  const handleManganese = (text) => {
    const value = parseFloat(text);
    if ((!isNaN(value) && value < 0) || value > 5) {
      setManganeseError("Manganese should be less than 5 ppm");
    } else {
      setManganeseError("");
    }
    setManganese(text);
  ...
  const handleNitrates = (text) => {
    const value = parseFloat(text);
    if ((!isNaN(value) && value < 0) || value > 5) {
      setNitratesError("Nitrates should be less than 5 ppm");
    } else {
      setNitratesError("");
    }
    setNitrates(text);
  ...
  const handleArsenic = (text) => {
    const value = parseFloat(text);
    if ((!isNaN(value) && value < 0) || value > 0.05) {
      setArsenicError("Arsenic should be less than 0.05 ppm");
    } else {
      setArsenicError("");
    }
    setArsenic(text);
  ...
  const calculate = () => {
    ...
    const CationResinVolume = vesselDetails?.resinFilled * 0.8;
    console.log("Cation Resin Volume:", CationResinVolume);
    ...
    if (vesselDetails && exchangeCapacity && hardness) {
      const totalHardnessCapacity =
        (exchangeCapacity * CationResinVolume * 50000) / 1000;
      ...
      const SaltRequiredRegeneration =
        (regenerationLevel * CationResinVolume) / 1000;
      ...
      const Backwash = vesselBackwashData
        ? vesselBackwashData.backwash * 1.5
        : null;
    ...
    <html>
      ...
      <body>
        ...
        <table>
          ...
          <tr><td>Iron (ppm)</td><td>${iron}</td></tr>
      ...
      >
        <TouchableWithoutFeedback onPress={closeDropdowns}>
          <View style={styles.card}>
            ...
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Iron (ppm (mg/ L))</Text>
              <TextInput
                style={styles.input}
                placeholder="Enter Iron "
                value={iron}
                onChangeText={handleIron}
                keyboardType="numeric"
              />
              {ironError ? (
                <Text
            ...
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Manganese (ppm (mg/ L))</Text>
              <TextInput
                style={styles.input}
                placeholder="Enter Manganese"
                value={manganese}
                onChangeText={handleManganese}
                keyboardType="numeric"
              />
              {manganeseError ? (
                <Text
            ...
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Nitrates (ppm (mg/ L))</Text>
              <TextInput
                style={styles.input}
                placeholder="Enter Nitrates"
                value={nitrates}
                onChangeText={handleNitrates}
                keyboardType="numeric"
              />
              {nitratesError ? (
                <Text
            ...
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Arsenic (ppm (mg/ L))</Text>
              <TextInput
                style={styles.input}
                placeholder="Enter Arsenic"
                value={arsenic}
                onChangeText={handleArsenic}
                keyboardType="numeric"
              />
              {arsenicError ? (
                <Text
