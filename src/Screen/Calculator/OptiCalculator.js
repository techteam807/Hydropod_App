...
const OptiCalculator = () => {
  ...
  const [residualChlorine, setResidualChlorine] = useState(null);
  ...
  const handleResidualChlorine = (text) => {
    const value = parseInt(text);
    if ((!isNaN(value) && value < 0) || value > 2) {
      setResidualerror("Please Enter flow less then 2 ppm");
    } else {
      setResidualerror("");
    }
    setResidualChlorine(text);
  };
  ...
  const calculate = () => {
    ...
    const CationResinVolume = vesselDetails?.resinFilled * 0.5;
      ...
      >
        <TouchableWithoutFeedback onPress={closeDropdowns}>
          <View style={styles.card}>
            ...
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Residual Chlorine (ppm (mg/ L))</Text>
              <TextInput
                style={styles.input}
                placeholder="Enter Residual Chlorine"
                value={residualChlorine}
                onChangeText={handleResidualChlorine}
                keyboardType="numeric"
              />
              {residualerror ? (
                <Text
